...
class feature_generator:
    def __init__(self):
        ...
        self.set_attrs(["v", "n", "ns", "nt", "nr", "a", "x", "p", "d", "b", "c"])
    ...
    def get_feature_str(self, pos, idx, words):
        ...
        for i in range(idx - self.num_neighbors, idx + self.num_neighbors + 1):
            ...
            if words[i].flag in self.attrs:
                feature_str += " %d:1" % ((pos - 1) * (len(self.attrs) + 1) + self.attrs[words[i].flag] + 1)
            else:
                feature_str += " %d:1" % ((pos - 1) * (len(self.attrs) + 1) + len(self.attrs) + 1 + 1)
            ...
            for j in range(i):
                ...
                if isTrain:
                    ...
                    for k in range(len(nrs)):
                        if (nrs[k].find(words[nr_idx].word)<0):
                            continue
                        if (nts[k].find(words[nt_idx].word)<0):
                            continue
                        p = 1
                        break
